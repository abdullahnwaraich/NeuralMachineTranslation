...
def pad_sents(sents, pad_token):
    ...
    """
    len_sent_lst=[len(sent) for sent in sents]
    max_len=max(len_sent_lst)
    sents_padded = []
    for j in range(len(sents)):
      sents_padded.append(sents[j]+(max_len-len(sents[j]))*[pad_token])
...
def test_pad_sents():
    sents = [["hello", "world"], ["hi"], ["greetings", "earthlings", "from", "mars"]]
    pad_token = "<pad>"
    expected_output = [
        ["hello", "world", "<pad>", "<pad>"],
        ["hi", "<pad>", "<pad>", "<pad>"],
    ...
    # Call the function
    actual_output = pad_sents(sents, pad_token)
    
    # Assert to check if actual_output matches expected_output
    assert actual_output == expected_output, f"Test failed! Expected {expected_output}, but got {actual_output}"
    
    print("Sanity test for pad_sents passed!")
